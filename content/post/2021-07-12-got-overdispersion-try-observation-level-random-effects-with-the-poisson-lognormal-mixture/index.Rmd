...
where the residual variance not accounted for by $x$ is captured in $\sigma$[^3]. There's a residual-variance term. The conventional Poisson model,
...
We might the `pp_check()` function to get a graphic sense of how well each model fit the data.
...
Even after accounting for model complexity, the Poisson-lognormal model appears to be the best fit for the data. Nest we consider how, exactly, does one interprets the parameters of the Poisson-lognormal model. 

### How does one interpret the Poisson-lognormal model?

A nice quality of both the conventional Poisson model and the gamma-Poisson model is their intercept corresponds directly with the mean of the original data, after exponentiation. The mean of the `SALAMAN` variable, recall, was `r round(mean(salamanders$SALAMAN), 1)`. Here are the summaries for their exponentiated intercepts.
...
These, recall, are several hundred credible lognormal distributions for the case-level $\lambda_i$ parameters, not for the data themselves. We'll get to the data in a moment. Since we're working with a multilevel model, we have posteriors for each of the case-level $\lambda_i$ parameters, too. Here they are in a dot plot.
...
At this point, though, you may be wondering how this model, with all its lognormal $\lambda_i$ glory, can inform us about actual counts. You know, the kind of counts that allowed us to fut such a wacky model. We'll want to work with the posterior draws for that, too.

Before we extract the posterior draws, let's save the posterior means for the $\mu$ and $\sigma$ parameters.

```{r}
mu <- posterior_summary(fit3)["b_Intercept", "Estimate"]
sigma <- posterior_summary(fit3)["sd_SITE__Intercept", "Estimate"]
```

Now extract all of the posterior draws for those parameters.
